...
class ProductCubit extends Cubit<ProductState> {
  ...
  void Myproduct() {
    emit(productmm(product));
  }

  void filteredpro(String categorie) {
    List<ProductModule> productbycat = product
        .where((element) =>
            element.productCategory.toLowerCase().contains(categorie))
        .toList();

    if (productbycat.isEmpty) {
      emit(Emptycategorie());
    } else {
      emit(categoriepro(productbycat));
    }
  }

  void filtered(String categorie) {
    List<ProductModule> productbycat = product
        .where(
            (element) => element.productTitle.toLowerCase().contains(categorie))
        .toList();
